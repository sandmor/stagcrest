...
fn biome_mood(albedo: vec3<f32>, ambient_color: vec4<f32>, day_factor: f32) -> vec3<f32> {
    ...
    return albedo * (vec3<f32>(1.0) + ambient * (0.5 + day_factor * 0.5)) * (0.35 + day_factor * 0.65 + night * 0.05);
}
...
fn sample_sky_direction(dir: vec3<f32>, lighting: SceneLighting) -> vec3<f32> {
    ...
    col *= 0.15 + 0.85 * lighting.params.x;
    ...
    let sun_glow = pow(sun_dot, 128.0) * max(lighting.params.x, sun_disc * 0.5) * sun_disc;
    ...
    let moon_glow = pow(moon_dot, 64.0) * (1.0 - lighting.params.x) * moon_disc * moon_above;
...
fn shade_voxel(
    ...
    let sky_contrib = sky_level * (sun + moon + 0.15) * (0.2 + 0.8 * day);
