...
impl MeshScheduler {
    ...
    pub fn is_busy(&self, pos: ChunkPos) -> bool {
        self.pending.contains_key(&pos) || self.in_progress.contains(&pos)
    }
}
...
pub fn mesh_rebuild_after_atlas_change(
    mut commands: Commands,
    pending: Option<Res<stagcrest_render::PendingMeshRebuild>>,
    mut scheduler: ResMut<MeshScheduler>,
) {
    let Some(pending) = pending else {
        return;
    };
    for pos in &pending.chunks {
        scheduler.request(*pos, RemeshUrgency::Visible, 0);
    }
    commands.remove_resource::<stagcrest_render::PendingMeshRebuild>();
}
